...
# Step 1: Decode quoted-printable text
decoded_text = quopri.decodestring(email_text).decode("utf-8", errors="ignore")

# Step 2: Identify categories (e.g., Breakfast, Lunch, Dinner, etc.)
category_pattern = re.compile(
    r"For (breakfast|lunch|dinner|snacking|dessert) this week, I'm making:",
    re.IGNORECASE,
)

# Step 3: Process the text into structured data
meal_plan = {}
current_category = None
current_recipe = None  # Store the last detected recipe title
...
for i, line in enumerate(lines):
    ...
    # Check if the line defines a new category
    match = category_pattern.search(line)
    if match:
        current_category = match.group(1).capitalize()  # Standardize category names
        meal_plan[current_category] = {}
        continue  # Move to next line

    # Detect Recipe Titles (Lines starting with "* ")
    if line.startswith("* "):
        current_recipe = line.lstrip("* ").strip()  # Extract title
        continue  # Move to next line

    # Detect Recipe Links (Lines containing "http" and inside parentheses)
    if current_recipe and "(" in line and "http" in line:
        raw_link = re.search(r"\(\s*(https?://[^\s)]+(?:=[\n\s]*[^\s)]+)*)\s*\)", line)
        if raw_link:
            cleaned_link = raw_link.group(1).replace("=\n", "").replace("=", "")

            # Store recipe title & link in the current category
            if current_category:
                meal_plan[current_category][current_recipe] = cleaned_link

            # Reset title to avoid duplicate matches
            current_recipe = None
